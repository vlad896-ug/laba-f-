...
namespace Files_and_Collections 
{
    ...
    class Program
    {
        static void Main()
        {
            ...
            List<string> L = SolutionMethods.FillList(quantity6);
            Console.WriteLine("Содержимое списка:");
            SolutionMethods.DisplayList(L);
            bool hasDuplicates = SolutionMethods.HasDuplicates(L);
            if (hasDuplicates)
            {
            ...
            LinkedList<string> linkedList = SolutionMethods.FillLinkedList(quantity7);
